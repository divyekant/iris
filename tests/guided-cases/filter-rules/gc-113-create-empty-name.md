...
2. **Attempt to create a rule with an empty name**
   ...
   - **Input**:
     ...
     ```
   - **Expected**: 400 Bad Request, response body contains an error message indicating name is required
   ```bash
   curl -4 -s -w "\n%{http_code}" -X POST http://127.0.0.1:3000/api/filter-rules \
     -H "X-Session-Token: $TOKEN" \
     -H "Content-Type: application/json" \
     -d '{"name":"","conditions":[{"field":"from","operator":"contains","value":"test"}],"actions":[{"type":"archive"}]}'
   ```

3. **Verify no rule was created**
   - **Target**: `GET /api/filter-rules`
   - **Input**: None (auth header only)
   - **Expected**: 200 OK, response does not contain a rule with an empty name
   ```bash
...
- [ ] Response contains an error message about the empty name
- [ ] No rule with an empty name exists in the GET list

## Failure Criteria
- POST returns 201 (rule was created despite empty name)
- POST returns a status code other than 400
- A rule with an empty name appears in the list
